# Реализация рулевого управления Акермана на базе 4х колёсной платформа ROS Noetic для имитационного моделирования в Gazebo

##  Аннотация

В данной работе рассматривает пример создания симуляциой модели четырёхколёсной мобильной платформы с рулевым управления по типу Аккреманна,  с использованием фреймворка `ROS`. В качестве среды симуляции выбрана `Gazebo`. Приведены примеры описания архитектуры робота, посредством формата `URDF`, разработан собственный контроллер дл управления колёсами модели, а также показан общий подход к проектированию симуляционных моделей робототехнических систем.
...
## Содержание 
- [Реализация рулевого управления Акермана на базе 4х колёсной платформа ROS Noetic для имитационного моделирования в Gazebo](#реализация-рулевого-управления-акермана-на-базе-4х-колёсной-платформа-ros-noetic-для-имитационного-моделирования-в-gazebo)
  - [Аннотация](#аннотация)
  - [Содержание](#содержание)
  - [Источники](#источники)
  - [Предварительные требования](#предварительные-требования)
  - [Создание пакета ROS](#создание-пакета-ros)
  - [Введение в геометрия Акерманна](#введение-в-геометрия-акерманна)
  ...
  - [Реализация контроллера рулевого управления Акермана](#реализация-контроллера-рулевого-управления-акермана)
...
- Установленный `ROS` (например, `ROS Noetic`)
- Созданный `ROS` пакет (например, `my_ackermann_controller`)
- `Python 2.7` или `Python 3.x`, в зависимости от версии `ROS`
...
Здесь я показал как выглядит мой пакер ROS, подробно о построении пакетов `ROS` можно ознакомиться в официальной документации. Всё это необходимо для нормального запуска проекта.
...
## Введение в геометрия Акерманна

Геометрия Акермана является фундаментальным принципом в дизайне систем управления для четырехколесных транспортных средств, обеспечивая оптимальный поворот путем корректировки углов поворота передних колес. Этот принцип особенно актуален для мобильных роботов с рулевым управлением, где необходимо достичь эффективного, прогнозируемого и безопасного поворота.

__Основы геометрии Акермана__
Геометрия Акермана основана на идее, что все колеса должны быть ориентированы к центру поворотного круга во время поворота. Это уменьшает проскальзывание и износ колес, улучшая управляемость. Для достижения этого передние колеса поворачиваются под разными углами. В идеале линии, проведенные через оси вращения всех колес, должны пересекаться в одной точке на оси задних колес (изображения даны только лишь для ознакомления).

<img src="https://upload.wikimedia.org/wikipedia/commons/8/89/Ackerman_Steering_Linkage.gif
" alt="_wiki_ackermann_gif" height="400"/> | <img src="https://upload.wikimedia.org/wikipedia/commons/9/97/ExtendedAckermann.gif" alt="wiki_ackermann_img" height="400"/> 
...
Геометрия Акермана требует, чтобы углы поворота передних колес ($\theta_L$ для левого и $\theta_R$ для правого) были настроены так, чтобы выполнялось следующее условие:

$$ \frac{1}{R+\frac{T}{2}}=\frac{\tan(\theta_L)}{W}, \frac{1}{R-\frac{T}{2}}=\frac{\tan(\theta_R)}{W} $$
где:
- R - радиус поворотного круга транспортного средства,
- W - расстояние между осями ведущих колёс и ведомых колёс,
- T - ширина между центрами колёс на одной оси,
- $\theta_L, \theta_R$ -  углы поворота для левого и правого передних колес соответственно.

__В робототехнике__

В контексте мобильных роботов, геометрия Акермана позволяет точно определить углы поворота передних колес для заданного радиуса поворота. Это особенно важно при проектировании алгоритмов управления, где необходимо достигнуть плавного и точного поведения робота при маневрировании.

[К содержанию](#содержание)

## Создание имитационной URDF модели робота (Xacro)

Создание любой имитационной модели в `ROS` начинается с проектировании структуры, описывающей конструкцию робота в формате `urdf`: звенья, соединения, их масса-инерционные характеристики, другие физические свойства, а также трансмиссия. 

Проектирование `URDF` (Unified Robot Description Format) файла для робота с системой управления Ackermann может быть довольно сложной задачей, поскольку требует учёта множества деталей, включая физические размеры, веса, и расположение компонентов робота. Для простоты, мы спроектируем небольшого робота на четырёх колёсах, где передние колёса управляются согласно принципу управления Акерманна. Это базовое описание можно дальше расширять и модифицировать в соответствии с конкретными требованиями  проекта.

Общепринято роботизированные системы описывать в формате `urdf`, однако для описания некоторых сложных конфигураций можно использовать `sdf`, очень похожий формат, с некоторыми отличиями. Принцип формирования конечного файла: `xacro -> urdf -> sdf`. Таким образом в конченом итоге `ros` рабоатет с sdf файлом. Для ознакомления с доступными тегами при описании файла можно обратиться к [документации]((http://sdformat.org/spec)).
...
- Суставы для вращения колёс: Все колёса (и передние, и задние) должны иметь соединения, позволяющие им вращаться для имитации движения. Это также могут быть соединения типа `continuous`.
...
Для увеличения масштабируемости и гибкости проекта выполним небольшую декомпозицию структуры робота, а именно создадим несколько файлов в директории `urdf` (xacro):
...
- [`corpus.xacro`](https://github.com/AntonSHBK/custom_ackermann_steering_controller_ros/blob/main/urdf/corpus.xacro) - файл, писывающий корпус.
...
Такой принцип декомпозиции юлы принят из соображения: каждый уникальный элемент структуры должен быть описан отдельно от других, а непосредственно сборка описан отдельным файлом (как в машиностроении - детали, сборочные единицы).

Для  разрабатываемой структуры робота примем просты формы геометрии: в качестве корпуса - параллелепипед, в качестве колёс - цилиндры. В дальнейшем можно заменить просты формы на более детализированные модели в формате `stl` или `dae`, если это необходимо
...
Здесь мы описали описали звенья и соединения (взаимное расположение звеньев). При этом мы использовали перченные, это позволяет гибко и быстро выполнять конфигурирование проекта.

Для дальнейшей работы с этим описанием, например, для добавления датчиков, двигателей или для настройки физических свойств (вес, инерция и т.д.), файл URDF будет требовать соответствующего расширения и модификации.

Добавим определение трансмиссии для соединений. Трансмиссия свяжет соединение с контроллером через определённый интерфейс. 

Чтобы добавить трансмиссии для каждого колеса в `URDF` файл робота с рулевым управлением Акерманна, необходимо расширить определение робота, включив в него элементы `<transmission>`. Эти элементы связывают суставы (`<joint>`) с актуаторами через интерфейсы `hardware_interface`, позволяя `ros_control` управлять физическими или симулированными актуаторами.

Эти блоки `<transmission>` добавляются в конец  `URDF` файла, после определений всех суставов (`<joint>`) и перед закрывающим тегом `</robot>`. Они указывают, что для каждого колеса используется своя трансмиссия, связывающая суставы с актуаторами через определенные интерфейсы. Для передних управляемых колес используется `PositionJointInterface`, что подразумевает управление позицией (углом поворота), а для задних ведущих колес — `VelocityJointInterface`, управляющий скоростью вращения колес.

Вынесем описание трансмиссий в `common_macro.xacro` так мы уменьшим количество код и увеличим читаемость.
...
После добавления всех необходимых суставов и передач, `URDF` модель  готова к использованию с `ros-controls` для реализации механизмов управления. Ещё один важным элементом является добавление в описание робота информацию для `gazebo` (`gazebo_ros_control`)
...
```
Этот конфигурационный файл задает контроллер состояния суставов (`joint_state_controller`), который публикует состояние всех суставов робота. Также определены контроллеры для каждого из передних и задних колес. Для передних колес используются контроллеры позиции (`JointPositionController`), а для задних — контроллеры скорости (J`ointVelocityController`), что соответствует их функциям в системе под рулевым управлением типа Акерманна.

Применение, ошибка связанная с `pid` контроллера для симуляции в `Gezebo` не существенна. Если вы используете `pid` контролеры почитайте официальную [документацию](https://wiki.ros.org/pid).
```
...
Такой подход облегчает управление конфигурациями и позволяет легко адаптировать ваше ПО под различные условия эксплуатации без необходимости изменения исходного кода.

[К содержанию](#содержание)

## Реализация контроллера рулевого управления Акермана

- [Publisher and Subscriber Python tutorial ROS 1](https://wiki.ros.org/ROS/Tutorials/WritingPublisherSubscriber%28python%29)
- [ROS messages](https://wiki.ros.org/msg)
- [Geometry messages](https://docs.ros.org/en/api/geometry_msgs/html/index-msg.html)

Для реализации рулевого управления типа Акерманна для `ROS` и в интеграции с `ros-controls`, создадим собственный контроллер: определим взаимосвязи, а также определим логику управления колёсами. Для этого создадим скрипт на `python`. Определим новый класс, в которой инициализируем `ROS` ноду. Обязательно необходимо выполнить чтение параметров из системы, которые мы определяли на предыдущем шаге. Далее нашу ноду подпишем на топик управления в данном случае `/cmd_vel` и с некоторой периодичностью будем считывать с неё сообщения. Имея топике команды, реализуем логику расчёта геометрии Акерманна, а именно: определим функцию расчёта улов поворота колёс, функцию расчёт скорости приводных колёс.
...
Не забудьте сделать скрипт исполняемым, если будете запускать скрипт вручную, несовместно с `rosrun`:
...
Этот скрипт инициализирует `ROS`-узел, создает публикатор для топика `/cmd_vel`. Этот скрипт использует библиотеку `curses` для считывания нажатий клавиш и публикует сообщения `Twist` в топик `/cmd_vel`. Используйте стрелки для управления скоростью и направлением движения, `'s'` для остановки и `'q'` для выхода.
...
Завершающим этапом является подготовка файла запуска (`launch`), однако стоит поправить что этот файл подготавливается в написания всего проекта, на каждом его этапе, дополняя и расширяя его.

Файл запуска (`launch`-файл) в `ROS`играет важную роль в управлении запуском и конфигурацией узлов, параметров, пространств имен и других элементов системы. Это `XML`-файлы, которые предоставляют механизм для автоматизации сложного процесса запуска множества узлов и настройки их взаимодействия. 
...
<!-- *********************************************************************** -->
<!-- Запуск ноды контроллера Акерманна -->
<node 
...
Здесь должно быть всё достаточнопонятно, мы вызываем написанные наим ранее файлы запуска `rviz.launch` и `gazebo,launch`, а также запускаем ноду контроллера рулевого управления Аккермана.
...
Данная реализация имитационной модели не идеальная и является примером реализации подобной методологии. В дальнейшем модель можно усложнить, интегрировать в неё доугие модули (комеру, датчики расстояний и д.р.). В работе возможны ошибки и недочёты, буду рад вашей оценки со стороны.

## Контакты для связи со мной:
* [Telegramm](https://t.me/antonSHBK);
