...
Здесь должно быть всё достаточно нопонятно, мы вызываем написанные нами ранее файлы запуска `rviz.launch` и `gazebo,launch`, а также запускаем ноду контроллера рулевого управления Аккерманна.
